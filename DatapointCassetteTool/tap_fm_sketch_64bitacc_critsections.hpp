/* 
 * tap_fm_tx_sketch.hpp (refactored with conditional debug + 64-bit accumulator-only buffering + ISR-safe updates)
 * --------------------------------------------------------------------------------------------------------------
 ...
 *     with extra slack from a 64-bit accumulator instead of a FIFO, and
 *     ISR-safe updates to shared state.
 */
...
// ========================================================
  ...
  // Provide noInterrupts()/interrupts() as no-ops in Linux shim
  // so the same code compiles both on MCU and in tests.
 ...
 *  - Protected by disabling interrupts while updating shared state
 *    (bitAccumulator and bitsInAcc) so ISR cannot see torn values.
 */
static inline void fmPushEncodedBit(
    ...
    volatile uint8_t  &bitsInAcc,      // number of valid bits in bitAccumulator (0–64)
    ...
    // Update accumulator and bit count atomically w.r.t. ISR
    noInterrupts();
    bitAccumulator |= (uint64_t(out1) << bitsInAcc++);
    bitAccumulator |= (uint64_t(out2) << bitsInAcc++);
    interrupts();

    // Update last line bit (1 byte, safe as volatile)
    lastLineBitRef = out2;
}
 ...
 *  - Uses fmPushEncodedBit internally (which is interrupt-safe).
 */
...
// 64-bit accumulator for FM bits (LSB-first). Volatile: shared with ISR.
static volatile uint64_t bitAccumulator = 0ULL;

// Number of valid bits currently stored in bitAccumulator (0..64). Volatile: shared with ISR.
static volatile uint8_t  bitsInAcc      = 0;
...
//  - All updates to bitAccumulator/bitsInAcc happen inside fmPush*
//    which are interrupt-safe.
// ============================================================
static void produceFmBits() {
    ...
    while (true) {
        // ---- INTERRUPT-SAFE CHECK OF bitsInAcc ----
        bool room;
        noInterrupts();
        room = (bitsInAcc <= 42);
        interrupts();
        ...
        if (!room) {
            break;   // Not enough room for 22 FM bits → exit
        }
