...
int main () {
  ...
  while (fgets(buffer, 1024, stdin)) {
    if (cnt!=0) {
      ...
      if (instructionSet[set*256+(0xff&opCodeNum)].type != instructionTypeNum) {
        printf ("opcode=%03lo set=%d is not matching %d:%ld\n", opCodeNum, set, instructionSet[set*256+(0xff&opCodeNum)].type, instructionTypeNum);
        exit(1);
      }
      if (strcmp(instructionSet[set*256+(0xff&opCodeNum)].mnemonic, mnemonic)!=0) {
        printf ("opcode=%03lo set=%d is not matching %s:%s\n", opCodeNum, set, instructionSet[set*256+(0xff&opCodeNum)].mnemonic, mnemonic);
        exit(1);
      }
